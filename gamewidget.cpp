...
GameWidget::GameWidget(QWidget *parent) : QWidget(parent)
{
    ...
    QStringList bgNames = {"bg_church.png","bg_dungeon.png","bg_pyramid.png",
                            "bg_volcano.jpg","bg_library.jpg","bg_church.png"};
    for (const QString &n : bgNames) sceneBg << QPixmap(resDir + "/" + n);

    QStringList fbNames = {"bg_church_fb.png","bg_dungeon_fb.jpg","bg_pyramid_fb.png",
                            "bg_volcano_fb.jpg","bg_library_fb.jpg"};
    for (const QString &n : fbNames) sceneBgFlashback << QPixmap(resDir + "/" + n);

    QColor whiteBg(255,255,255);
    playerIdle = loadTransparent("player_idle.png", whiteBg, 60);
    playerWalkR << loadTransparent("player_walk_r1.png", whiteBg, 60)
                << loadTransparent("player_walk_r2.png", whiteBg, 60)
                << loadTransparent("player_walk_r3.png", whiteBg, 60);
    playerWalkL << loadTransparent("player_walk_l1.png", whiteBg, 60)
                << loadTransparent("player_walk_l2.png", whiteBg, 60)
                << loadTransparent("player_walk_l3.png", whiteBg, 60);
...
QString GameWidget::findResDir() const
{
    ...
    for (const QString &c : candidates) {
        if (QFile::exists(c + "/bg_church.png")) return c;
    }
    return appDir;
}
...
void GameWidget::syncFlashbackBg()
{
...
void GameWidget::handleDialogEnd()
{
    ...
    if (endedKey == "church_statue") {
        statueTalked = true;
    }
    if (endedKey == "church_circle") {
        circleTalked = true;
    }
    if (endedKey == "church_candle") {
        candleTalked = true;
    }
...
void GameWidget::checkCollisions()
{
    ...
    // ---- 场景0：教堂 ----
    if (player.sceneId == 0) {
        QRect circleR(450, 580, 128, 40);
        QRect statueR(300, 480, 100, 140);
        QRect candleR(150, 520, 80, 100);

        if (returnedFromLibrary && player.hasProp("seed") && interactRect.intersects(circleR)) {
    ...
    if (player.sceneId == 1) {
        ...
        if (interactRect.intersects(dreamR)) {
            ...
            } else if (seedDiscovered) {
                startDialog("dream_machine_with_seed");
            } else if (!dreamMachineUnlocked) {
                dreamMachineUnlocked = true;
                startDialog("dream_machine");
            } else {
                startDialog("dream_machine");
            }
            return;
...
void GameWidget::drawTitle(QPainter &p)
{
    ...
    if (!loaded) { titleBg.load(resDir + "/title_bg.png"); loaded = true; }
    ...
    QString poem = "山河有尽，文明有痕，有缘之人，循迹而来";
    QFontMetrics fmPoem(p.font());
    int poemW = fmPoem.horizontalAdvance(poem);
    p.drawText((width() - poemW) / 2, 70, poem);

    // ========== 左侧文字 ==========
    int leftX = 80;
    int leftY = height() / 2 - 60;

    // 左侧花纹
    p.setPen(QColor(120, 100, 70, 150));
    p.setFont(QFont("SimHei", 10));
    p.drawText(leftX, leftY - 30, "✦ ───────────── ✦");
    p.drawText(leftX, leftY + 80, "✦ ───────────── ✦");
    ...
    p.drawText(leftX, leftY, "千年迟赴");

    // 左侧副标题
    p.setPen(QColor(180, 170, 140, 200));
    p.setFont(QFont("SimHei", 13));
    p.drawText(leftX, leftY + 35, "文明灰烬 · 圣女之约");

    // ========== 右侧文字 ==========
    int rightX = width() - 280;
    int rightY = height() / 2 + 80;

    // 右侧花纹
    p.setPen(QColor(120, 100, 70, 150));
    p.drawText(rightX, rightY - 50, "✦ ───────────── ✦");
    p.drawText(rightX, rightY + 60, "✦ ───────────── ✦");
    ...
    p.drawText(rightX, rightY, "按 空格键");

    p.setPen(QColor(160, 160, 160, int(60 + blink * 120)));
    p.setFont(QFont("SimHei", 12));
    p.drawText(rightX, rightY + 25, "开始游戏");

    // ========== 底部装饰线 ==========
    p.setPen(QColor(100, 90, 70, 100));
    p.drawLine(60, height() - 40, width() - 60, height() - 40);

    // 角落花纹
    p.setPen(QColor(140, 120, 80, 120));
    p.setFont(QFont("SimHei", 16));
    p.drawText(30, 50, "✧");
    p.drawText(width() - 50, 50, "✧");
    p.drawText(30, height() - 30, "✧");
    p.drawText(width() - 50, height() - 30, "✧");
}
...
void GameWidget::drawScrollingBg(QPainter &p)
{
    ...
    if (showingFlashbackBg && sceneId < sceneBgFlashback.size() && sceneBgFlashback[sceneId].isNull()) {
        QString fbFile = resDir + "/bg_church_fb.png";
        if (sceneId == 1) fbFile = resDir + "/bg_dungeon_fb.jpg";
        else if (sceneId == 2) fbFile = resDir + "/bg_pyramid_fb.png";
        else if (sceneId == 3) fbFile = resDir + "/bg_volcano_fb.jpg";
        else if (sceneId == 4) fbFile = resDir + "/bg_library_fb.jpg";
        QPixmap fb(fbFile);
        if (!fb.isNull()) {
...
void GameWidget::drawProps(QPainter &p)
{
    ...
    if (player.sceneId == 0) {
        ...
        static bool loaded = false;
        if (!loaded) {
            magicCircle = removeBackground(QPixmap(resDir+"/prop_magic_circle.png"), blackBg, 30);
            magicCircleGlow = removeBackground(QPixmap(resDir+"/prop_magic_circle_glow.png"), blackBg, 30);
            churchStatue = QPixmap(resDir+"/prop_church_statue.png");
            churchCandle = removeBackground(QPixmap(resDir+"/prop_church_candle.jpg"), blackBg, 40);
            portalDoor = removeBackground(QPixmap(resDir+"/prop_portal_door.png"), blackBg, 30);
            loaded = true;
        }
        QPixmap &circle = (seedDiscovered || returnedFromLibrary) && !magicCircleGlow.isNull() ? magicCircleGlow : magicCircle;
        if (!circle.isNull()) p.drawPixmap(450, 580, 128, 40, circle);
    ...
    if (player.sceneId == 1) {
        static QPixmap dreamMachine, holoScreen;
        static bool loaded = false;
        if (!loaded) {
            dreamMachine = removeBackground(QPixmap(resDir+"/prop_dream_machine.png"), blackBg, 30);
            holoScreen = removeBackground(QPixmap(resDir+"/prop_holo_screen.png"), blackBg, 30);
            loaded = true;
        }
        if (!dreamMachine.isNull()) p.drawPixmap(500, 440, 200, 200, dreamMachine);
    ...
    if (player.sceneId == 2) {
        ...
        static bool loaded = false;
        if (!loaded) {
            wallPaint = removeBackground(QPixmap(resDir+"/prop_wall_painting.png"), blackBg, 30);
            stoneTablet = removeBackground(QPixmap(resDir+"/prop_stone_tablet.png"), blackBg, 30);
            stoneCoffin = removeBackground(QPixmap(resDir+"/prop_stone_coffin.png"), whiteBg, 30);
            loaded = true;
        }
        if (!stoneTablet.isNull()) p.drawPixmap(350, 490, 80, 100, stoneTablet);
        if (!wallPaint.isNull()) p.drawPixmap(550, 490, 200, 120, wallPaint);
        if (!stoneCoffin.isNull()) p.drawPixmap(830, 540, 120, 80, stoneCoffin);

        if (coffinUnlocked) {
            p.setPen(Qt::NoPen);
            p.setBrush(QColor(80,150,255,50));
            p.drawEllipse(870, 580, 160, 120);
        }
    }
    ...
    if (player.sceneId == 3 && swordDropped) {
        ...
        static bool loaded = false;
        if (!loaded) {
            brokenSword = QPixmap(resDir+"/prop_broken_sword.png");
            loaded = true;
        }
    ...
    if (player.sceneId == 4) {
        ...
        static bool loaded = false;
        if (!loaded) {
            bookshelf = removeBackground(QPixmap(resDir+"/prop_bookshelf.png"), whiteBg, 30);
            manuscript = removeBackground(QPixmap(resDir+"/prop_manuscript.png"), whiteBg, 30);
            seedVessel = removeBackground(QPixmap(resDir+"/prop_seed_vessel.png"), whiteBg, 30);
            seed = removeBackground(QPixmap(resDir+"/prop_seed.png"), whiteBg, 30);
            portalDoor = removeBackground(QPixmap(resDir+"/prop_portal_door.png"), blackBg, 30);
            loaded = true;
        }
    ...
    if (player.sceneId == 5) {
        ...
        if (!magicCircleGlow.isNull()) p.drawPixmap(450, 580, 128, 40, magicCircleGlow);
    }
}
...
void GameWidget::drawNpcs(QPainter &p)
{
    ...
    for (auto &npc : scene.npcs()) {
        ...
        if (id == "well") {
            ...
            static bool loaded = false;
            if (!loaded) {
                wellBlocked = removeBackground(QPixmap(resDir+"/prop_well_blocked.png"), whiteBg, 30);
                wellOpen = removeBackground(QPixmap(resDir+"/prop_well_open.png"), whiteBg, 30);
                loaded = true;
            }
            ...
            if (!loaded) {
                ...
                portalImg = QPixmap(resDir+"/prop_portal_door.png");
                loaded = true;
            }
            ...
            static bool loaded = false;
            if (!loaded) {
                trapped = removeBackground(QPixmap(resDir+"/npc_lobster_trapped.png"), whiteBg, 60);
                saved = removeBackground(QPixmap(resDir+"/npc_lobster_saved.png"), whiteBg, 60);
                loaded = true;
            }
...
void GameWidget::drawDialog(QPainter &p)
{
    ...
    bool showHero  = (speaker == "勇者");
    bool showKing  = (speaker == "国王");
    bool showGhost = (speaker == "守剑幽魂");

    static QPixmap heroImg, kingImg, ghostImg;
    static bool portraitLoaded = false;
    if (!portraitLoaded) {
        heroImg  = QPixmap(resDir + "/portrait_hero.png");
        kingImg  = QPixmap(resDir + "/portrait_king.jpg");
        ghostImg = QPixmap(resDir + "/portrait_ghost.png");
    ...
    int boxX = 60, boxY = height() - 280, boxW = width() - 120, boxH = 240;
    ...
    if (showHero && !heroImg.isNull()) {
        int ph = 280;
        int pw = qMin(200, int(ph * (float)heroImg.width()/heroImg.height()));
        p.drawPixmap(30, height()-ph-40, pw, ph, heroImg);
    }
    if (showKing && !kingImg.isNull()) {
        int ph = 280;
        int pw = qMin(200, int(ph * (float)kingImg.width()/kingImg.height()));
        p.drawPixmap(30, height()-ph-40, pw, ph, kingImg);
    }
    if (showGhost && !ghostImg.isNull()) {
        int ph = 280;
        int pw = qMin(200, int(ph * (float)ghostImg.width()/ghostImg.height()));
    ...
    p.setFont(QFont("SimHei", 12));
    int textTop = boxY + 40;
    int textHeight = (state == GameState::Choice) ? 110 : 180;
    ...
    if (state == GameState::Choice && !currentDialog.choices.isEmpty()) {
        ...
        for (int i = 0; i < currentDialog.choices.size(); i++) {
            ...
            p.drawRect(boxX+30, y+i*26, boxW-60, 22);
            p.setPen(QColor(230,230,220));
            p.setFont(QFont("SimHei", 11));
            p.drawText(boxX+40, y+i*26+16,
...
void GameWidget::drawEnding(QPainter &p)
{
    ...
    static bool loaded = false;
    if (!loaded) {
        endingBgs[1].load(resDir + "/ending_seed.png");
        endingBgs[2].load(resDir + "/ending_return.png");
        endingBgs[3].load(resDir + "/ending_time.png");
        endingBgs[4].load(resDir + "/ending_dream.png");
        endingBgs[5].load(resDir + "/ending_coffin.png");
        loaded = true;
    }
...
void GameWidget::drawPixelGround(QPainter &p)
{
    ...
    if (!loaded) {
        ...
        tileLibrary.load(resDir + "/tile_library.png");
