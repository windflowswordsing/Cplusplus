...
void Scene::load(int sceneId)
{
    ...
    if (sceneId == 0) {
        ...
        // NPC
        m_npcs << Npc("well_entrance", 80, 520, 60, 100, QColor(50, 45, 40), "古井入口");

        // 出口
        m_exits << SceneExit{QRect(80, 520, 60, 100), 1, 100, 300, ""};           // 古井→地下城
        m_exits << SceneExit{QRect(1050, 480, 64, 140), 4, 100, 300, "broken_sword"}; // 石门→图书馆（需断剑）

        // 回溯触发
        m_flashbackTrigger = "magic_circle";
        m_hint = "按 E 交互 | A/D 移动 | 空格 跳跃";
    }
    else if (sceneId == 1) {
        // ===== 场景二：赛博朋克地下城 =====
        m_name = "赛博朋克地下城";
        ...
        // 道具
        m_props << Prop("dream_machine", 550, 440, 100, 60, QColor(60, 70, 90), "幻梦机器");
        ...
        // 出口
        m_exits << SceneExit{QRect(50, 580, 40, 40), 0, 100, 300, ""}; // 古井通道→教堂

        m_flashbackTrigger = "lobster";
        m_hint = "按 E 交互 | 小心幻梦机器";
    }
    else if (sceneId == 2) {
        // ===== 场景三：沙漠金字塔 =====
        m_name = "沙漠金字塔";
